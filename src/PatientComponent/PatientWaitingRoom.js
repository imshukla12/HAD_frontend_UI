  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/appointment/waitingPatients/${appointmentId}`
      );
  ...
  return (
    <>
      ...
      <div className="bg-blue-50 p-16 h-screen w-full items-center justify-center">
        <h1 className="text-5xl font-serif text-center">Waiting Area</h1>
        <br />
        <h3 className="text-lg font-serif text-center">
        ...
        <div className="border-2 border-gray-500 rounded-lg h-4/5 flex flex-row items-center justify-center space-x-16 p-2">
          ...
          <div className="justify-center items-center flex flex-col gap-8 h-full w-1/6 p-8">
            ...
            <div className="items-center justify-center">
              <button
                type="button"
                className="text-white w-full bg-green-500 hover:bg-green-700 font-serif text-lg rounded-lg text-sm px-2 py-2 text-center mr-3 md:mr-0 transform transition duration-300 hover:scale-110"
